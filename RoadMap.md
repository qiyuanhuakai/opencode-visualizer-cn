...
- [ ] 修复已知问题
    ...
    - [x] acp：会话不显示左下角的持续时间
    - [x] opencode：模型、提供商和状态监控概率被其他后端信息污染；目前状态监控-插件显示codex信息
    - [x] all：子代理历史加载概率存在问题（不显示标题和历史信息），需要手动刷新页面才出现
    - [x] all：子代理历史可能/或许会在子代理发送消息时才出现，而不是在子代理ses创建时就出现
    - [x] all：会话状态更新失败：在同一会话中，无法从“思考中”正确回到idel/无法从idel切回“思考中”。但是切个新会话旧会话好像就可以回去
    - [x] renderder：在超大文件(App.vue)中，行评论功能无法正常工作，滚动表现似乎与小文件不同
    - [x] 在输入框中按上按键弹出的历史记录中存在大量assistant的消息。这里应当只包含user发送的内容
