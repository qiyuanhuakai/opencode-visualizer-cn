...
- [x] 兼容迁移已有历史：所有会话写入成功后才精简原文件，迁移失败时保留原数据；退出时等待资源清理与待写入数据落盘，避免部分任务失败导致其他写入提前中断。
- [x] 保护尚未保存的本地输入，避免跨窗口存储事件或其他会话的草稿更新覆盖正在输入的文字，修复输入回退与吞字。
- [x] 切换会话时丢弃过期渲染任务与缓存；渲染失败时显示经过转义的原始文本并结束等待，后续更新可重新尝试渲染。
- [x] 为会话加载后的滚动定位增加超时兜底，避免动画帧暂停时持续显示加载中。
- [x] 补充存储迁移、输入保护、渲染恢复与退出清理回归测试及 Windows Electron 实机检查脚本；在隔离的真实历史副本上验证 370 个会话迁移内容一致，小配置写入由修复前的 2.24–3.88 秒降至 5.9–8.9 毫秒。

### Kimi Web 命令与旁支会话

- [x] 将子智能体默认模型、思考强度和独立的 tower 实验开关收进模式旁的紧凑设置菜单；下拉选项适配 Vis 主题，并移除无用的子会话查看入口。
- [x] 新增 `/new`、`/clear`、`/btw`、`/fork`、`/undo`、`/status`、`/subagent` 和 `/copyall`，保留 `/compact` 作为上下文压缩入口；`/copyall` 分页读取并复制完整会话 Markdown，历史记录补齐工具调用、Shell 和网络搜索。
- [x] 每次 `/btw` 都新建独立旁支窗口：窗口起始为空白，模型仍继承主会话上下文，窗口内可继续追问；修复已完成回复仍显示“思考中”的问题。
- [x] 更新 `DESIGN.md`：新页面必须适配 Vis 视觉与当前主题，下拉选择使用 Vis 主题化组件，不使用系统原生下拉框。
...
### Kimi Web 2.0.2 会话界面与交互

- [x] 提供商与模型改为紧凑列表，连接操作进入二级界面；manual/auto/yolo 保留原名和颜色，plan/swarm/tower 收入向上展开的「模式」菜单。
- [x] 会话按项目与沙盒整理并支持固定；底部提供线程目标、子智能体、压缩和复制入口。消息卡片支持分支、差异、撤销，仅在该轮确有文件变更时显示「差异」，运行中的卡片即使被排队消息取代也会在轮次结束后刷新。
- [x] 状态监控按会话状态显示模型、上下文和 Token 用量，并统一 Kimi 额度、插件与提示的主题样式；子智能体可配置默认模型，支持可用的 Kimi Web 斜杠命令。
- [x] 文件树显示隐藏文件和 Git 忽略文件，被忽略项使用灰色文字；修复打开文件内容为空，并将 Shell 接入 bridge PTY。未运行过的会话保持灰色空心状态，完成轮次后才标为 Idle。
- [x] 分支、压缩、撤销按当前连接的实际能力探测展示，插件列表探测失败时禁用相关操作；修复子会话忙闲查询和编辑器文件名的 Shell 参数传递。

### Kimi Web 回归与 CI

- [x] PR #139 完整测试通过 3,296 项、跳过 6 项，lint、类型检查及构建通过；真实 Kimi Web 与浏览器场景覆盖会话、文件、Shell 和卡片操作。CI 将 Vitest worker 限为 2 个，解决并行负载下集成测试触发 5 秒超时的问题；更新后所有 CI 检查通过。
