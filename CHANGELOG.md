...
- [x] 输入框为 `\` 序列提供与 `/`、`@` 一致的补全交互；Space、Tab 或 Enter 会完成替换，弹层打开时优先采用当前高亮项，未匹配内容保持原文。光标移动、程序化草稿更新和中文输入法组合输入均不会触发错误替换或误发送。
- [x] 点击发送或程序化发送时会执行同一转换兜底；匹配支持常见及 Unicode 标点边界、大小写折叠和重复项最后一条生效，同时不会把 `\high`、`\hi_suffix` 等更长标识符误识别为较短序列。
