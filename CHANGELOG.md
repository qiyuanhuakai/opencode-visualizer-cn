...
- [x] 浮动窗口创建时会把陈旧或桌面尺寸与坐标收敛到可见 canvas 内；实时拖动和缩放不再受 canvas、屏幕边缘或输入区限制，`pointerup`、`pointercancel` 或 capture 丢失时统一清理手势，结束后仅独立校准真正越界的坐标轴并保留至少 32px 可拖标题栏，避免阻尼、跨轴回弹和残留 listener 造成随机跳动。
- [x] 修复页面仍在加载、floating extent 暂为 `0×0` 时打开终端等窗口会被压成无高度横条且无法恢复的问题；窗口保留请求尺寸，并在首个有效 extent 到达时执行一次创建布局；异步 `beforeOpen` 采用同 key 最新请求生效语义，关闭窗口不会再被旧请求复活。
