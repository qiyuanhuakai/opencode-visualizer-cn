...
- [x] bridge 与上游密钥不再留在后台进程参数、状态文件或它启动的命令、PTY 与 ACP 子进程环境中；需要自动重启时可使用环境变量或 token 文件。
- [x] 通过系统 `PATH` 调用安装后的 `vis_bridge` 也能正常启动守护进程；Windows 升级或卸载会确认旧进程树已经退出，失败时停止安装而不是静默遗留旧进程。
- [x] bridge 会拒绝超大请求、异常 WebSocket 帧、无响应或伪造的上游握手，并断开持续积压输出的 PTY 客户端，避免长期运行时被异常连接拖垮。
