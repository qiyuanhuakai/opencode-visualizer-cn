...
### Kimi Web Integration (Alpha)

- [x] Add kimi web as the fourth backend: REST and WebSocket traffic is forwarded through vis_bridge `/kimi-web/*` with the kimi bearer injected lazily from `~/.kimi-code/server.token`, so the browser only ever holds bridge credentials, an established WebSocket survives token rotation, and rotation takes effect without a bridge restart.
- [x] Wire the main chat surface on top of the kimi web WebSocket protocol: session create/select with the required profile model write, streaming text and thinking deltas, history reload with reverse pagination and injection filtering, approvals and questions with authoritative list reconciliation, rename/archive/restore/delete/abort, steer, attachments, token usage, and the status monitor.
- [x] Open the tool, reasoning, and subagent floating windows automatically from the live event stream, honoring the existing suppress-auto-popup setting; recover disconnects with cursor replay plus snapshot rebuild.
- [x] Gate every entry point behind runtime capability probing (`meta.capabilities` plus `auth.models_ready`, fail-closed) and supervise `kimi web` with the three-state port contract (spawn, adopt, error without spawning).
- [x] Record the measured adaptation status, protocol limits, and replay boundary contract in `docs/kimi.md`, and list the integration in both README feature tables.
...
- [x] Fast 状态位于思考强度与 Goal 之间，支持命令、点击和键盘切换并保留输入草稿；服务档位遵循模型目录与服务器默认值。裸 `/plan` 切换 Plan/Default，携带任务时进入 Plan 并发送。
- [x] `/init` 使用 Codex `rust-v0.154.0` 官方原文，并在发布包中附带 Apache 许可证与来源说明。

### Codex 侧聊与会话操作修复

- [x] `/btw`、`/side` 创建独立临时侧聊，保留主会话运行及历史；创建时捕获模型、服务档位与权限，避免切换主会话后影响侧聊权限，审批和问题按所属会话隔离。
- [x] 关闭侧聊时仅中断并取消订阅侧聊；中断或取消订阅失败后保留重试状态，覆盖关闭后才收到启动响应的情况。
- [x] `/new`、`/fork`、`/archive` 导航前消费来源会话中的命令草稿，并保留附件与目标会话草稿；导航失败时按当前输入状态恢复命令。
- [x] 修复 `/review` 启动与完成状态的卡片归属；分页历史撤销使用原生 `thread/revert`，移除审查卡片并保留此前记录，同时刷新同会话 Goal 状态。撤销仅作用于会话记录，不回退工作区文件。
...
- [x] Codex 文件树显示被 `.gitignore` 忽略的文件与目录，并沿用 OpenCode 的灰色文字样式；正确处理忽略规则例外、已跟踪文件及特殊文件名。
- [x] 修复从 Codex 快速切换到 OpenCode 后误报“在此服务器上未找到该会话。”的问题：清除旧会话链接参数，并阻止过期的后端初始化结果覆盖当前状态，无需刷新即可使用新后端。
- [x] 修复重启 Vis 后冷加载 Codex 历史时，消息卡片多次猛烈向下跳动的问题：保留初始消息窗口，等待 Markdown 渲染与布局稳定后完成底部定位，加载期间暂停自动跟随。

### Codex 协作模式与连接恢复

- [x] 模型选择器按协作模式显示名称、颜色和简短说明，消息卡片同步显示并持久化发送时的协作模式。
- [x] 登录 Codex 后优先恢复设置中的模型与思考强度，避免被模型列表首项覆盖；补齐子代理订阅恢复与助手消息自动弹窗，保留历史加载静默、会话隔离和禁止自动弹窗设置。

### 测试组织与回归验证

- [x] 按职责拆分 Codex API、协议、设置和 SSE 等测试套件，复用组件、会话与传输夹具，统一释放作用域、计时器、Worker、连接和临时进程；为原有 300 个测试文件的 2,521 个场景建立完整保留与替代映射。
- [x] 通过最小内部抽取，以真实 App、Electron 主进程、组件交互和浏览器验证替换脆弱的源码文本断言与整页快照，覆盖历史加载、鉴权取消、PTY 所有权、存储、退出清理及子代理自动弹窗，保持产品行为和公开接口不变。
- [x] 新增测试维护指南与场景映射检查器，提供 Codex、ACP live、自动弹窗和 UI 验证入口；ACP live 要求显式指定专用会话与预期状态，并在结束后恢复配置，默认测试发现规则与并发设置保持不变。

## [v0.7.17 released]

### Codex 消息信息与历史操作

- [x] 按用户消息持久化发送时的模型与提供商信息，刷新、任务完成和后续切换模型时保持原有归属，避免退回 `codex` 占位名称；缓存按后端连接端点隔离。
- [x] 历史用户卡片支持撤销该消息及之后的回合，同一回合内的补充消息不重复计数；每张有差异的卡片独立显示“差异”，重新加载历史后仍保留，分支入口仅在最新卡片显示。

### Codex 子代理信息与悬浮窗

- [x] 适配 `subAgentActivity` 事件，补齐代码审查等子代理的信息与历史入口；代理名称只显示路径最后一级，模型信息读取子代理自身配置。
- [x] 订阅子代理及其后代的实时记录，思考、shell 和助手消息分别接入现有自动悬浮窗，遵循禁止自动弹窗与完成后延时关闭设置；历史加载不重放旧窗口，子代理内容不混入父会话，切换会话或重连后的过期响应不会干扰新订阅。

### Codex 浏览器历史缓存

- [x] 将浏览器中的思考与工具辅助历史缓存迁移至 IndexedDB，避免占满 localStorage 后，已完成卡片的思考强度、模型等信息保存失败并在刷新后消失；确认迁移事务成功后才释放原空间，失败时保留原数据并支持重试，桌面端沿用原生存储。
- [x] 辅助历史的写入和删除支持跨标签页同步；补充真实浏览器回归，覆盖存储满额、历史完整迁移、失败回退、刷新恢复及标签页同步。
...
- [x] 消息卡片沿用 OpenCode 的现有标题组件与主题，在模型名称右侧显示已知的思考强度；按线程和用户消息保留发送时的值，回传、补充消息和刷新后不被后续选择覆盖，兼容旧回合缓存，无法确定的历史保持留空。

### Codex 会话状态与消息历史

- [x] 刷新页面后恢复运行中的回合及中断目标，底部状态栏与顶部会话选择器保持一致；较旧的异步响应不再覆盖最新运行状态。
- [x] 本连接发送过消息或观察到运行的会话，结束后保留绿色空闲标记；未参与的会话保持灰色空心圆，参与记录刷新后保留，并按后端连接端点隔离。
- [x] 实时助手消息保持与所属用户请求的关联，减少每次操作触发的记录刷新；同一卡片中的多条助手消息保留独立身份并按时间顺序显示，不再覆盖前文、挤到历史记录顶端或脱离用户卡片。
- [x] 运行中发送补充消息时保留每条用户消息的独立身份，相同文字或图片的多次发送也保持独立；后续回复归属最新请求，此前启动、延迟完成的助手或工具记录仍归属原请求，前台和后台记录在切换会话、刷新后保持正确分组与顺序，发送失败的临时卡片会被移除。
- [x] 修复图片发送后重复显示、助手消息附件暂时显示为损坏的问题；图片读取失败时保留已正常显示的发送预览。
- [x] 历史记录展示 Codex 传输的思考记录，并支持读取子代理会话历史；分别处理思考摘要与原始内容，兼容既有辅助历史缓存。

### Codex 悬浮窗交互

- [x] 再次打开已有主面板或子面板时恢复最小化、置前并聚焦，保留用户调整的位置和尺寸，不重复解析窗口内容；自动更新不抢焦点，并保留最新 z-index，避免覆盖用户刚刚执行的置前操作。
...
- [x] 保留仅在完成时收到的思考与子代理内容，完成后的窗口保留约 3 秒再关闭，避免工具移除或回合结束导致最终内容丢失、窗口提前关闭；后续无思考内容的回合不重新弹出旧思考，保留 OpenCode 的现有接入方式。
...
- [x] 补充重连缓存、异步打开竞争、状态恢复和补充消息归属回归测试；通过真实浏览器验证首发、刷新恢复历史及悬浮窗恢复，并使用生产组件与模拟协议验证会话状态、消息分组和响应式显示。
