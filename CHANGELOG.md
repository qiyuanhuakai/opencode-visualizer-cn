...
- [x] 内置 CodeMirror 编辑器提供覆盖保存、历史、搜索、行操作、注释、折叠与补全的可录制快捷键；编辑器字体大小在字体设置中独立配置，默认继续继承现有代码字体大小。
- [x] Electron 桌面端可配置本地应用打开文本文件，并将应用中的连续保存安全写回当前后端；Web 端不显示该配置和入口。
- [x] 本地应用写回支持编辑器常见的原子替换保存，按后端与文件路径串行处理并在远端内容已变化时拒绝覆盖。
