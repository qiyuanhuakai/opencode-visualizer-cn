...
- [x] 运行中发送补充消息时保留每条用户消息的独立身份，后续助手回复归属最新请求；此前启动、延迟完成的助手或工具记录仍归属原请求，刷新后保留卡片分组与顺序。
- [x] 修复切换会话后后台补充消息的思考与工具记录错误归属第一条请求的问题，并移除发送失败的临时卡片；兼容既有辅助历史和思考强度缓存，相同文字或图片的多次发送保持独立。
