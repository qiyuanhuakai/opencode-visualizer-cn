...
- [x] Electron 使用主进程原子文件存储 Snippet 配置，并为旧 localStorage 数据提供失败可重试的批量迁移；写入失败、跨窗口更新与冲突解决不会静默覆盖草稿或外部数据。
- [x] 持久化与剪贴板 IPC 仅接受受信主窗口发送者，preload 只暴露受限接口；Chromium 沙箱、上下文隔离、导航与子窗口限制保持开启。

### Bridge 稳定性

- [x] `vis_bridge stop` 在返回 HTTP 202 前同步关闭新命令准入；已连接但尚未提交完整正文的请求会收到明确的 shutdown 400 响应，且不会启动子进程。
