 ...
 * @param gutterLines - Optional custom gutter labels (indexed by line number)
...
export function batchToRows(
  ...
  const stableRows = stableLines.length > 0
    ? buildRows(stableLines, mode, gutterLines).split('\n')
    : [];
  ...
  if (batch.unstable.length > 0) {
    ...
    // The unstable line's gutter index is the count of stable lines (0-indexed)
    const unstableGutterIndex = stableLines.length;
    // Build gutterLines array for the unstable line only, using the global index
    const unstableGutterLines = gutterLines
      ? [gutterLines[unstableGutterIndex] ?? String(unstableGutterIndex + 1)]
      : [String(unstableGutterIndex + 1)];
