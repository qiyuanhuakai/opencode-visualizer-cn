...
export function createKimiWebAgentClient(request: AgentRequest) {
  ...
  return {
    async listChildSessions(sessionId: string): Promise<KimiWebSession[]> {
      const items = new Map<string, KimiWebSession>();
      for (const busy of [false, true]) {
        const cursors = new Set<string>();
        let beforeId: string | undefined;
        do {
          const page = await request<KimiWebPage<KimiWebSession>>({ method: 'GET', path: `${path(sessionId)}/children`, query: { page_size: 100, before_id: beforeId, busy } });
