<template>
  <Dropdown class="kimi-composer-actions" :disabled="disabled" :label="t('common.more')" :auto-close="true" menu-icon="lucide:chevron-up" :popup-style="{ top: 'auto', bottom: 'anchor(top)', marginTop: '0', marginBottom: '6px', width: 'max-content', minWidth: '180px' }" @select="select">
    ...
    <DropdownItem v-if="compactAvailable" value="compact" :disabled="disabled || busy"><Icon icon="lucide:minimize-2" :width="14" :height="14" />{{ t('topPanel.sessionActions.compact') }}</DropdownItem>
    <DropdownItem v-if="forkAvailable" value="fork" :disabled="disabled || busy"><Icon icon="lucide:git-fork" :width="14" :height="14" />{{ t('topPanel.sessionActions.fork') }}</DropdownItem>
  </Dropdown>
</template>
...
const props = defineProps<{ disabled?: boolean; busy?: boolean; compactAvailable?: boolean; forkAvailable?: boolean }>();
...
function select(value: unknown) {
  ...
  switch (value) {
    ...
    case 'compact': if (!props.busy && props.compactAvailable) emit('compact'); return;
    case 'fork': if (!props.busy && props.forkAvailable) emit('fork'); return;
  }
}
</script>
