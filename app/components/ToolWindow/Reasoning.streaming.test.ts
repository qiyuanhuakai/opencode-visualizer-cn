...
function mountReasoning(initialEntries: Array<{ id: string; text: string; completed?: boolean }>): HTMLElement {
  const target = document.createElement('div');
  document.body.appendChild(target);
  const app = createApp(
    defineComponent({
      setup() {
        return () => h(Reasoning, { entries: initialEntries });
      },
    }),
  );
...
describe('Reasoning streaming prop', () => {
