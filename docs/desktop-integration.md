...
- 应用与已连接的 `vis_bridge` 分别检查版本并展示状态；bridge 安装包在本机下载和执行。
...
- 「安装器已打开」不是「安装成功」。Bridge 卡片显示当前已连接 bridge 的 `/healthz` 返回的版本，不回退为本机版本；旧 bridge 未提供版本字段时明确提示不可用，切换或断开连接后清除旧版本。更新检查、下载与安装仍针对本机，bridge 的当前版本以渲染进程上报的连接版本为准（而非 `vis_bridge --version`），并将可用下载标为「本机安装包可用版本」。
