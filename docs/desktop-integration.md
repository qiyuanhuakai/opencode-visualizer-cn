...
- 「安装器已打开」不是「安装成功」。Bridge 卡片显示当前已连接 bridge 的 `/healthz` 返回的版本，不回退为本机版本；旧 bridge 未提供版本字段时明确提示不可用，切换或断开连接后清除旧版本。更新检查、下载与安装仍针对本机，使用 `vis_bridge --version` 进行内部比较，并将可用下载标为「本机安装包可用版本」。
